	...
	public class Editor : UnityEditor.Editor
	{
		...
		private void DrawInspectedMembers(string label, object target, List<string> path, bool root = false)
		{
			...
			if (foldoutStates[key])
			{
				...
				// Collect all members that may be displayed. Private or hidden members are left out
				// in a later step.
				...
				foreach (var member in members)
				{
					...
					if (member is System.Reflection.FieldInfo)
					{
						...
						// A field will be shown if it is public or has a ShowInInspector attribute
						// and not has an HideInInspector attribute.
					...
					else if (member is System.Reflection.PropertyInfo)
					{
						...
						// A property will be shown if it has an getter method and if it has an
						// ShowInInspector attribute. For technical reasons is must be public.
					...
					else if ((type_.IsClass || type_.IsInterface) && !(value is string))
					{
						// Non primitive types are recursively displayed with a new foldout and
						// indent level in the inspector.
